	...
	<view class="wrap">
		<view class="u-tabs-box u-margin-top-20" style="padding: 0 20upx;">
			...
			<view class="searchbox">
				<u-search class="search_input" v-model="keyword" placeholder="搜索面题" :disabled="false" shape="square"
					:show-action="false" :animation="true" :clearabled="true" @search="search" @clear="clear">
				</u-search>
				<!-- 通知显示 -->
				<view class="u-padding-10" style="position: relative;" @click="toUrl('/pages/center/news')">
			...
			<!-- 选项卡 
			 ...
			 activeColor 激活颜色
			 current为初始化tabsSwiper的激活tab索引，默认为0
			 -->
		...
		<scroll-view scroll-y style="height: 85%;width: 100%;background: #f3f3f3;" @scrolltolower="reachBottom">
			<view class="page-box">
				<view class="order" @tap="goDetail(res.id)" v-for="(res, index) in list[current].list" :key="index">
					<view>
						<view class="item">
							<view class="content">
								<!-- 用户信息 -->
		...
		<u-popup v-model="releaseShow" mode="bottom" border-radius="20" length="30%">
			...
			<view class="">
				...
				<view class="fabu_xinzi" offset="1" span="10" @click="toUrl('/pages/salary/release')">
					<view class="release_item">
